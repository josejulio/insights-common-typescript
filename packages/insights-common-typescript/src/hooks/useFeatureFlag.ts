...
export type FeatureFlagSignature = {
    <T>(resolver: () => boolean, ifTrue: FeatureFlagCallback<T>, ifFalse?: FeatureFlagCallback<T>): T | undefined;
    <T>(resolver: () => boolean, ifTrue: FeatureFlagCallback<T> | undefined, ifFalse: FeatureFlagCallback<T>): T | undefined;
};
